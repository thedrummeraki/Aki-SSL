...
import aki.packages.utils.SignUtils;
...
public class Signable implements Dumpable {
    ...
    public Signable() {
        type = FileType.PEM;
        String tempHex = aki.packages.tools.FileWriter.dumpFilename(5, true, "");
    ...
    public void setContents(String contents) {
        ...
        aki.packages.tools.FileWriter.write(contents, contentsFilename);
    }
    ...
    public void setCertSigner(Certificate certSigner) {
        if (certSigner != null) {
            ...
            aki.packages.tools.FileWriter.write(blob, signerFilename);
        }
    ...
    public void setPrivateKeySigner(PrivateKey privateKeySigner) {
        if (privateKeySigner != null) {
            ...
            aki.packages.tools.FileWriter.write(blob, privateKeyFilename);
        }
    ...
    public void setData(String data) {
        if (data != null) {
            aki.packages.tools.FileWriter.write(data, signedFilenamePEM);
        }
        signedDataPEM = data;
    ...
    private void createFilename(int length, boolean hex) {
        this.filename = aki.packages.tools.FileWriter.dumpFilename(length, hex, null);
    }
    ...
    public int sign(Certificate signer, PrivateKey privateKey, String sigAlg) {
        Logger.info(TAG, "Signing the data...");
        ...
        if (!SignUtils.check(this)) {
            Logger.error(TAG, "Please make sure you have initialized the following:\n  - Some data to sign;\n  - The certificate signer;" +
        ...
        if (code != 0) {
            Logger.error(TAG, "Please make the sure the certificate and the private key match:" +
        ...
        if (code != 0) {
            Logger.error(TAG, "Error while signing the data.");
            return code;
        }

        SignUtils.setSignedData(this);
        Logger.info(TAG, "Signing the data was successful.");
        return 0;
    }
    ...
    public boolean sign() throws CertificateException {
        ...
        if (!aki.packages.tools.FileWriter.write(this.contents, tempRawData.getAbsolutePath())) {
...
//        addTempFile(tempRawData);
        ...
        if (!aki.packages.tools.FileWriter.write(this.certSigner.getBlob(), tempSignerBlob.getAbsolutePath())) {
        ...
        if (!aki.packages.tools.FileWriter.write(this.privateKeySigner.dumpPEM(this.certSigner.getSubject()), tempKey.getAbsolutePath())) {
        ...
        this.signedDataDER = BashReader.toSingleString(aki.packages.tools.FileReader.getLines(signedFilenamePEM)).trim().getBytes();
...
//        }
        ...
        this.signedDataPEM = BashReader.toSingleString(aki.packages.tools.FileReader.getLines(signedFilenamePEM)).trim();
        this.isSigned = true;
...
//        }
    ...
    public int verify() throws SignatureException {
        ...
        aki.packages.tools.FileWriter.write(this.getDERSignedDataAsString(), tempSigned.getPath());
...
//                    Integer.toString(offset), "+", Integer.toString(header), "]", String.format("count=%s", length)};
        ...
        if (!aki.packages.tools.FileWriter.write(this.certSigner.getBlob(), tempSigner.getAbsolutePath())) {
            throw new SignatureException("Couldn't write the signer's blob of data to the file.");
        }
        ...
        aki.packages.tools.FileWriter.write(br.getOutput(), tempSignerPubKey.getPath(), false);
        ...
        aki.packages.tools.FileWriter.write(this.contents, sha1sum.getPath());
