...
public class Certificate implements Dumpable {
    ...
    public PublicKey fetchPublicKey(String inform) {
        ...
        if (br != null && br.getExitValue() == 0) {
            aki.packages.tools.FileWriter.write(br.getOutput(), publicKeyFilename);
        } else {
...
//            Logger.debug("PKCS7", "Issuer: "+iss);
    ...
    public String getModulus() {
        ...
        if (!aki.packages.tools.FileWriter.write(getBlob(), tempCert.getPath())) {
    ...
    private void writeToFilename() {
        ...
        aki.packages.tools.FileWriter.write(this.blob, filename);
    }
    ...
    public static Certificate loadCertificateFromFile(String filename) throws CertificateException {
        ...
        if (certificate != null) {
            ...
            certificate.blob = BashReader.toSingleString(true, aki.packages.tools.FileReader.getLines(filename));
        }
        return certificate;
    }
